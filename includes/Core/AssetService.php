		...
		wp_interactivity_state(
			...
			array(
				'i18n' => array(
					...
					// Cover-photo remove dialog + follow-request toasts. Read by
					// profile/store.js via t() but never injected, so they always
					// rendered English. coverRemoved is profile-specific ("Cover photo
					// removed"); the spaces namespace has its own "Cover removed.".
					'removeCoverTitle'                 => __( 'Remove cover photo?', 'buddynext' ),
					'removeCoverBody'                  => __( 'Your cover photo will be cleared. You can upload a new one any time.', 'buddynext' ),
					'coverRemoved'                     => __( 'Cover photo removed', 'buddynext' ),
					'coverRemoveFailed'                => __( 'Could not remove your cover. Try again.', 'buddynext' ),
					'followRequested'                  => __( 'Follow request sent', 'buddynext' ),
					'followRequestWithdrawn'           => __( 'Follow request withdrawn', 'buddynext' ),
				),
			)
		);
	}
		...
		wp_interactivity_state(
			...
			array(
				'i18n' => array(
					...
					// Bulk decide on join requests. Read by notifications/store.js via
					// t() but never injected, so they always rendered English.
					'bulkDecideFailed'    => __( 'Could not update those requests.', 'buddynext' ),
					/* translators: 1: number handled, 2: number no longer pending. */
					'bulkDecidedPartial'  => __( '%1$d handled, %2$d no longer pending.', 'buddynext' ),
					/* translators: %d: number of members approved. */
					'bulkApproved'        => __( '%d members approved.', 'buddynext' ),
					/* translators: %d: number of requests declined. */
					'bulkDeclined'        => __( '%d requests declined.', 'buddynext' ),
				),
			)
		);
	}
		...
		wp_interactivity_state(
			...
			array(
				'i18n' => array(
					...
					// rendered English.
					'fillUsername'     => __( 'Please choose a username.', 'buddynext' ),
					'awaitingApproval' => __( 'Your account is awaiting administrator approval.', 'buddynext' ),
				),
			)
		);
	}
