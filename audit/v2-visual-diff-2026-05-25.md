...
| Right sidebar — Profile Completion: circular gauge `82%` + `3 to go` + checklist (Cover ✓ / Bio + tagline ✓ / Skills 6 ✓ / Connect 3+ accounts / Verify email / Add pinned post) | Profile Strength card with `Add a bio / Add your work experience / Add your skills` action buttons | Different design language: proto uses circular-gauge primitive; live uses CTA list. Circular-gauge primitive not built. |
...
- **`parts/member-card.php`** supports `degree` (1st/2nd pill), `mutual_count`, `bio`, `member_type_label`, presence, and the full Follow / Connect / Message / mute cluster.
...
**Real remaining gaps (not "all of E"):** inline Follow on the **post byline**; **mutual-avatar pile** (member-card has the count, not the avatar stack); **member number** (no field — low value). The directory/feed look sparse mostly because **accounts have empty profiles**, not because the templates lack the fields — a content gap, as this doc's own profile table already noted.
