...
| Date | Branch | Change |
|---|---|---|
| 2026-06-30 | 1.0.4 | Space discovery + auto-join (plan: `docs/plans/space-discovery-and-auto-join.md`). **Suggestions:** `Spaces/SpaceSuggestionService` ranks spaces a viewer might join from social-proof (`bn_follows`→`bn_space_members`) + category-affinity + popularity, hydrated visibility-safe via `SpaceService::list_spaces()` (new `include_space_ids`/`exclude_space_ids` args), diversity-capped, cached per viewer (`SpaceSuggestionListener` busts on join/leave/follow). Exposed at `GET /buddynext/v1/spaces/suggestions` and surfaced as the directory "Suggested for you" rail (inside the interactive region, outside `[data-bn-sd-grid]`) + the onboarding step-2 candidate source. **Auto-join (owner-curated, going-forward):** two Permissions-panel settings — `auto_join_on_signup` (boolean core field) + `auto_join_member_types` (plain comma-meta filter, NOT a registered field to avoid an always-on member-type query + the multiselect option-sanitizer stripping values). `Spaces/AutoJoinService` + `AutoJoinListener` join eligible spaces on `user_register` (no type filter) and `buddynext_member_type_assigned` (type in filter) via idempotent `SpaceMemberService::join()`. Bulk backfill of existing members deferred (would storm the join hook's notification/webhook consumers; needs an AS-batched, notification-suppressed path). Also fixed in the same pass: F5-a reactive join/leave toggle (stale Interactivity binding after `swapButtonState` → `routeMembership` routes by live `data-current-state`), F7-a settings save bar dead on Permissions/Moderation/Notifications (missing `savebarMarkDirty` trigger), F1-b reactive 0-result blank grid (always-present empty scaffold), F1-a seed categories. WPCS + PHPStan L5 clean throughout; per-surface browser-verified. |
| 2026-06-29 | 1.0.4 | Spaces R3 custom-fields settings panel (browser-verified). Additive (owner-scoped): the 8 built-in fields keep their bespoke tabs; R3 adds a registry-driven surface for third-party developer-registered space fields (the ones with no UI today). Added a `core` flag to `SpaceFieldRegistry::register()` (CoreSpaceFields marks its 8 `core=true`) + `get_custom_fields()`. New `templates/parts/space-settings-panel-fields.php` renders each non-core field via `Profile\FieldType::render_input()` (label/hint/required/inline-error), saved over `POST /spaces/{id}/fields` by the new `assets/js/space-fields/store.js` (`saveFields`: collect-by-shape, success toast, per-field 422 inline). Module registered in `AssetService` + i18n (`i18n_space_fields`); `PageRouter` enqueues it on the settings sub-page. A "Custom fields" tab is added in `templates/spaces/settings.php` only when `get_custom_fields()` is non-empty (no empty tab for the typical owner). Verified live with temp-registered url+select fields: render, save→`bn_space_meta`, invalid-option 422 inline; core fields excluded; WPCS+PHPStan-L5 clean; 390px/dark. See `docs/plans/spaces-master-plan.md` "R3 — as built". Remaining: R4 (search-fold, optional). |
| 2026-06-29 | 1.0.4 | Spaces R2 member panel → REST (browser-verified). Converted `templates/parts/space-settings-panel-members.php` from 5 server POST forms to the existing `buddynext/space-members` Interactivity store (same store the Members tab uses). Reused `removeMember`/`changeRole`; added `banMember` (`POST /spaces/{id}/bans`) + `inviteMember` (`POST /spaces/{id}/invite`, bound to form submit so Enter works, `preventDefault`, success clears+toasts no-reload) to `assets/js/space-members/store.js` + i18n in `AssetService::i18n_space_members`. `PageRouter` enqueues the module on the settings sub-page too (was members-tab only). Removed the legacy POST handler in `templates/spaces/settings.php` (+ dead `$save_error_message`/`invite_sent` notice). Verified live: promote/demote/remove/ban/invite all fire REST + persist; zero legacy forms; WPCS+PHPStan-L5 clean; 390px/dark. Also: `DemoDataService` now seeds 5 sub-spaces (`SUBSPACES`) under Design Critique + Frontend Guild so the parent<->child link is demonstrable on a fresh seed. See `docs/plans/spaces-master-plan.md` "R2 — as built". Remaining: R3 (web field panel), R4 (optional). |
| 2026-06-29 | 1.0.4 | Spaces R1 frontend Nav UX (browser-verified). Three SSR additions, no new REST (reads the same SpaceService data the app layer exposes): (1) **breadcrumb** in `templates/parts/space-hero.php` — `Parent > This space` above the `<h1>` when a space has a parent, via `SpaceService::parent_summary()` (visibility-scoped, omitted when the parent is unseeable), parent link via `buddynext_space_url()`, `.bn-sh-hero__breadcrumb` tokens-only/dark-safe with ellipsis; (2) **Sub-spaces rail card** in `templates/parts/space-sidebar.php` registered on `buddynext_right_sidebar` (placed in the persistent right rail — the Discord/Notion expectation — not the Feed tab body), lists `get_subspaces()` children (visible list, sidesteps the secret-child count gap) as linked emblem+name+`_n()`-count rows, only on a root space; (3) **manager-only Add sub-space CTA** (gated on `buddynext-manage-space` + the `buddynext_space_allow_sub` toggle) opening `create-space-modal.php`'s new `$fixed_parent` path — title "Create a sub-space", locked `Sub-space of <name>` chip + hidden `parent_id` (consumed by the existing `submitCreate`), modal wrapped in its own `data-wp-interactive="buddynext/spaces"` region so its actions bind. Verified end to end (CTA→locked modal→create→`parent_id` set→redirect→breadcrumb+rail update), WPCS + UX-audit clean, 390px no-overflow, dark token-flip. Right rail is desktop/tablet-only by existing template design (all cards) — mobile parent context carried by the hero breadcrumb. See `docs/plans/spaces-master-plan.md` "R1 — as built". Remaining Spaces frontend: R2 (member panel→REST), R3 (web field panel), R4 (optional). |
| 2026-06-29 | 1.0.4 | Spaces gap-audit fixes (post-T8 double-check). Closed two pre-existing "saved-but-not-applied" contract bugs the orphan-field sweep surfaced: (1) `SpaceMemberService::can_invite()` now honours the per-space `who_can_invite` setting (members\|mods\|owner) using the same role-rank threshold as `who_can_post` — it previously hardcoded owner/mod/admin and ignored the setting; (2) the space's `default_notification_pref` is now applied to a new member's `bn_space_members.notification_pref` on every creation path (`join`/`request_join`/`invite`) via one shared `default_notification_pref()` helper — it was previously written by settings but never read, so members always got `all`. Both defaults (`mods`, `all`) preserve prior behavior. Documented (not regressions): Pro `MembershipAdmin` per-space `_paywall_*` options should migrate to `bn_space_meta` (Pro follow-up); sub-space `total` vs secret-child visibility; missing archived-success notice; deferred directory composite index. See `docs/plans/spaces-master-plan.md` Gap-audit. phpcs/PHPStan-L5 clean. |
| 2026-06-29 | 1.0.4 | Sub-spaces app layer + move/detach + admin completeness (T10+T11). `SpaceService::get_subspaces()` (index-backed by the v11 `parent` key, bounded, secret-child-scoped), `count_subspaces()`, `parent_summary()`, and `validate_parent_move()` (depth/cycle/cap/manage-new-parent/has-children guards). REST: `GET /spaces/{id}/subspaces` (paginated, perm-aware); `GET /spaces/{id}` now returns `parent{id,name,slug}` (breadcrumb) + `subspace_count`; `PUT /spaces/{id}` accepts `parent_id` to move/detach (respects the error's own 404/422/403). Directory `roots_only` filter in `list_query_scope` (SSR `directory.php` + REST `list_spaces` both pass it for the top-level browse — children are reached from their parent or via search). Admin `Admin/Spaces.php`: Pending column (batched count, no N+1), Archive/Unarchive row action + `handle_archive` + `bn_archive_space` nonce, and a "Sub-space of X" hierarchy label (batched parent names). T10 app layer + directory filter verified end-to-end (create→list→breadcrumb→root-filtered browse→search; EXPLAIN uses `parent`); move/detach + admin are code-verified (phpcs/PHPStan-L5) pending browser verification. Remaining Spaces frontend (nav breadcrumb/section/CTA, member panel→REST, web field panel) specced in `docs/plans/spaces-master-plan.md`. |
| 2026-06-29 | 1.0.4 | Space fields foundation + built-in options dogfooded (T7+T8). New `bn_space_meta` table (WP-meta-shaped: `meta_id`/`bn_space_id`) is the per-space metadata substrate behind the native API: `$wpdb->bn_spacemeta` alias wired in `buddynext.php`, `get/add/update/delete_space_meta()` quartet, `buddynext_register_space_field()` + `SpaceFieldRegistry` (registers `register_meta('bn_space')` on the `buddynext_register_space_fields` init action, mirroring the Nav API), `FieldType::display_text()`/`rest_value()` formatters. REST app layer: `GET /spaces/fields` (schema), `POST /spaces/{id}/fields` (manage-gated, atomic, 422 on invalid), `GET /spaces/{id}` returns visibility-filtered `fields[]` with value+display. **Pro collision resolved**: Pro had shipped `bn_space_meta` with a bespoke `(id, space_id)` schema; `Installer::maybe_reshape_space_meta()` converges any pre-existing table to canonical BEFORE dbDelta (copying `space_id`→`bn_space_id`, zero data loss) — Free now owns the table (Pro cutover is T7-Pro). **T8**: the 8 built-in per-space options (`push_to_feed`, `mvs_media_tab`, `jetonomy_forum_id`, `require_join_approval`, `who_can_post`, `who_can_invite`, `banned_words`, `default_notification_pref`) are now core space fields via `CoreSpaceFields`; all 10 readers + writers moved off autoloaded `bn_space_{id}_*` options to `get_space_meta()`/`buddynext_get_space_field()` (single read accessor, default from the field def); `FeedService::feed_excluded_space_ids()` replaced the `alloptions` scan with an indexed `bn_space_meta` query; `Installer::maybe_migrate_space_options()` migrates existing option values → meta and deletes them (autoload P0 closed); `SpaceService::delete()` clears `bn_space_meta` rows. Verified end-to-end: migration (options→meta, deleted), typed REST read/write, 422 validation, 401 auth, FeedService query, web settings render — all on the real DB via the real `admin_init` migration path. phpcs/PHPStan-L5 clean. |
| 2026-06-29 | 1.0.4 | Spaces scale indexes (schema v10→11). Added `KEY parent (parent_id)` on `bn_spaces` and `KEY space_status (space_id, status, joined_at)` on `bn_space_members` so sub-space lookups and space-roster pagination stay index-backed at 50k members/space. Fresh installs get them inline in CREATE TABLE; existing installs via a new idempotent `ADD KEY` loop + `index_exists()` helper in `Installer::maybe_alter_tables()` (mirrors the `column_exists` pattern; guarded by INFORMATION_SCHEMA.STATISTICS). Verified end-to-end via the real `admin_init` migration path: EXPLAIN shows the roster query on `space_status` (range, backward index scan, no filesort) and the sub-space count on `parent` (ref, covering "Using index"). phpcs/PHPStan-L5 clean. |
