...
// The summary strip shows reactions only — comment and share counts live on
// their action buttons. It is rendered even at zero reactions (hidden below), so
// the client can reveal and fill it the moment the FIRST reaction lands rather
// than the chip only appearing on the next full render (C8.6). The trigger's
// Interactivity bindings are SSR-present here so they hydrate normally.
...
do_action( 'buddynext_part_post_reaction_summary_before', $args );
?>
<div class="<?php echo esc_attr( $bn_class ); ?>" aria-label="<?php esc_attr_e( 'Post summary', 'buddynext' ); ?>"<?php echo 0 === $bn_reaction_n ? ' hidden' : ''; ?>>
	<?php // Trigger + popover render even at zero reactions so a 0→1 reaction fills them client-side. ?>
		<?php
		...
		<?php endif; ?>
		<?php endif; /* $bn_reaction_n > 0 (empty trigger at zero reactions). */ ?>
		</button>
		<?php
