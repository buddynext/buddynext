...
|---|---|---|---|
| 1 | Activity home (`/activity/`) | prod | Composer ships Image / Poll / Event / Voice / AI helper + chip privacy popover (Public / Followers / Only me). Filter tabs (For you / Following / Spaces / Network) wired to `FeedService::home_feed( …, $filter )` + `/feed/counts`. Post card gains Share button → Repost / Quote / Copy-link modal. **Infinite scroll** wired through new `GET /feed/home/page` (server-rendered HTML chunks via `FeedController::render_items_html()` → canonical post-card partial), IntersectionObserver-driven, DOMParser append (no innerHTML, no script execution), inline Retry on fetch error, end-of-feed marker on last page. Post-card timestamps now wrap a permalink anchor pointing at `/p/{id}/`. Share modal Copy-link copies the short permalink. Sidebar widgets gain per-row Follow chip, This-week caption, unread-space dot, empty states. |
| 1a | Single-post permalink (`/p/{id}/`) | prod | New surface — breadcrumb (Activity / @author / Post), full post card via canonical `partials/post-card.php`, expanded thread region anchored by `do_action( 'buddynext_single_post_thread' )`. Server-side visibility gates mirror `PostController::get_post()` exactly (blocks, secret-space, followers-only, private, suspended/shadow-ban) — all failures render the 404 state silently so existence is never leaked. New `SinglePostMeta` service emits canonical link, OG (type/url/title/description/image/site_name/article:author/published/modified), Twitter card (summary_large_image when an image resolves, summary otherwise), and `<meta name="robots" content="noindex,nofollow">` on private / followers / connections / space_members posts or posts inside private/secret spaces. Document `<title>` becomes `{author}: "{60-char excerpt}…"`. |
| 1b | Bookmarks hub (`/me/bookmarks/`) | prod | New surface — `/me/bookmarks/` rewrite resolves to `templates/feed/bookmarks.php`. Paginated saved-post list (15/page, cursor on `bookmark.created_at|id`), full-card render, visibility re-evaluated per row so privacy changes after bookmarking are honoured. "Bookmarks" entry added to the left-rail "You" group with `bookmark` icon and `aria-current="page"` when active. `GET /me/bookmarks` is backward compatible — default returns the legacy `{ids: int[]}` shape, `?expand=posts` returns paginated hydrated post records (`items`, `total`, `page`, `per_page`, `has_more`). Empty / loading / end-of-feed states all ship. |
| 2 | Activity explore (`/activity/explore/`) | prod | Composer + filter strip + Share modal + sidebar polish shared with Home via the same partials and stores; skeleton + inline error states ship for both surfaces. **Infinite scroll** parity with Home via new `GET /feed/explore/page` endpoint; sentinel + DOMParser append + end-marker work identically on the 3-column masonry grid. |
...
| 5 | Member profile view (`/members/varundubey/`) | prod | Title "Display Name · Profile" via PageRouter override; hero renders Headline, Location, Website (rel=nofollow ugc), Pronouns, Bio plus brand-coloured Social Link chips; inline Work Experience + Education timeline cards and Community Interests tag cloud render below the hero when populated; non-owner view ships Follow / Connect / Message / **Share** + More-options menu wired to Mute / Block / Report with toast feedback. **Owner-gate fixed** (was leaking Edit Profile / Avatar / Cover onto other members' profiles for admins) — strict `$is_own_profile` gate now applied in both the call site and the partial. The four hero stat cells (Posts / Followers / Following / Connections) are real navigation: Posts switches tab via `actions.setTab`, the other three link to `/members/{slug}/followers/`, `/following/`, `/connections/`. **Tabs URL-sync** via `history.pushState(?tab=…)` + popstate so deep links + browser-back work; the prior broken `.bn-ptab` selector replaced with the real `.bn-tab`. Replies / Media / Likes / Discussions tab content all populated. New Share popover: copy-link (clipboard + textarea fallback) + share-to-feed. 390px stacks the action row, locks the stats grid to 2x2. |
| 5a | Profile followers (`/members/{slug}/followers/`) | prod | New surface — header with count + back-to-profile link, 24 cards/page driven by `FollowService::followers()` with bidirectional block filtering, member-card row with View + state-aware Connect chip (via `partials/connection-button.php`) + Follow-back chip. Empty state copy adapts to own/other viewer. Document title "Followers · {name}". |
| 5b | Profile following (`/members/{slug}/following/`) | prod | New surface — same shape as followers, driven by `FollowService::following()`. Document title "Following · {name}". |
| 6 | Profile edit (`/members/varundubey/edit/`) | prod | All sections wrapped in a single `<form data-wp-on--submit=actions.saveProfile>`; sticky save bar carries master Save with dirty / saving / saved status pills; display_name is required and shows inline error on blur; website + social URLs validated client + server side with field-level 422 errors and toast on save; beforeunload guard fires when dirty; page title now "Edit Profile · Display Name". **New Privacy section** between Community Interests and Notification preferences carries three audience selects (see-email / DM / @mention — enum `everyone | members | connections | nobody`, 422 on invalid) and three toggles (directory visibility / search-engine indexable / hide-profile-views mirroring Pro P5.3). **`actions.togglePref` shipped** (was a silent noop on every existing notification toggle) — optimistic toggle with rollback + toast across every switch. **Account section beefed up** with inline change-email (new `POST /auth/change-email` → pending email usermeta + verification email via `buddynext_email_change_requested`), inline change-password with 5-step strength meter (new `POST /auth/change-password` → `wp_check_password()` validate + same-password + min-length + `wp_set_password()` + re-issue cookie so session survives), Sign-out-everywhere (new `POST /auth/sign-out-everywhere` → `WP_Session_Tokens::destroy_all()`). Notification-prefs card now has a footer CTA "Open notification preferences" linking to `/settings/notifications/`. |
| 7 | Spaces directory (`/spaces/`) | prod | Apply button dropped; filter bar is reactive (250 ms debounced search + instant type chips + sort chip-button popover). Secret chip added (scoped to viewer-owned memberships). Skeleton / empty / error states + retry all ship. Create-space CTA opens inline modal partial (`templates/partials/create-space-modal.php`) that POSTs to `/buddynext/v1/spaces`, surfaces field-level 422 errors and redirects on 201. REST list endpoint accepts `?type=open|private|secret`, `?orderby=popular|active|newest|alphabetical`, `?q=`, `?per_page=` capped at 50. **Wave 4** — type chips now read 'Open / Private / Secret' (was 'Public / Private / Secret') via the new `SpaceService::type_label()` helper. Empty state branches into cold-start ('No spaces yet' + Create CTA) vs filter-no-match ('No spaces match' + Reset filters CTA). Sub-480px sweep stacks the filter row vertically, sort trigger goes full-width. |
| 8 | Space home (per space) | prod | Hero ships cover, name, type badge, member count, owner avatars, 4-tile stat strip. Action row covers all 5 join states (Join / Joined / Pending / Request to join / not-logged-in) plus Notification-pref chip (bell) with All / Mentions only / None popover (optimistic + rollback), Invite button (owner/mod), Settings link (owner/mod), Moderation tab (owner/mod only). Feed scoped to space via existing FeedService query. Members tab renders the full active-member grid with role chips and per-row Follow buttons. About tab now runs description through `wp_kses_post( wpautop() )`. New REST: GET/POST `/spaces/{id}/notification-pref`, POST `/spaces/{id}/transfer` alias. **Wave 4** — document title now reads `'{Space name} · Spaces'` (or section-prefixed for `?bn_tab=members\|about` + `/settings/` + `/moderation/`) instead of the bare 'Spaces' hub fallback. Type pill renders 'Open' (not 'Public'). About tab renders House rules (numbered list from new `bn_spaces.rules` TEXT column, populated via Settings → General) plus a category chip linking to the filtered directory. Guest experience: secret-space slug 404s for non-members; private gate card branches into guest / pending / eligible; feed body shows a 'Join to participate' CTA in place of the composer for guests + non-member open-space viewers. Members tab gains All / Owners / Moderators / Members filter chips with per-filter empty messaging and a per-row Connect button next to Follow. Sub-480px sweep: hero emblem tightens, stat strip 2-up, tab nav scrolls horizontally, members grid 1-up. |
| 9 | Space settings (per space, owner) | prod | New tab set — General / Permissions / Members / Branding / Moderation / Integrations / Notifications / Danger zone. Permissions tab carries who-can-post (members/mods/owner) + who-can-invite + require-join-approval + require-post-approval toggles. Members list ships role chips + promote / demote / remove / ban with optimistic UI + toast + rollback. Branding tab shows a soft Pro upsell card (Pro P6.2 renders the real implementation via `buddynext_space_branding_settings`). Danger zone now ships Transfer ownership (target picker → POST `/spaces/{id}/transfer`) and Delete space behind a two-step gated confirm modal (`templates/partials/space-delete-confirm-modal.php`) that requires the typed space name + a matching `X-BN-Confirm-Space-Name` header on the DELETE call. PUT `/spaces/{id}/permissions` for inline saves. **Wave 4** — sticky save bar at the page footer mirrors Profile edit + Notification prefs (dirty / saving / saved status pills, captured-defaults rollback, beforeunload guard). Cover image upload now wires `wp.media()` properly: PageRouter enqueues media on the settings route, the field ships Upload / Remove actions, the preview tile paints the chosen URL as a background image, the hidden text input POSTs the URL back, and SpaceService caches are busted on save. General → House rules textarea persists to the new `bn_spaces.rules` column. Settings type-pill labels canonicalise on 'Open / Private / Secret'. New "Categories" admin subtab (`/wp-admin/admin.php?page=buddynext-spaces&subtab=categories`) lets non-CLI admins create + delete categories — previously the SpaceCategoryController REST endpoints had no UI and the category select was permanently empty. |
| 10 | Notifications (`/notifications/`) | prod | Every type renders human copy via `NotificationMessageService` (30+ types covered, spec at `docs/specs/NOTIFICATION-MESSAGES.md`). REST list response now ships `message`/`url`/`icon`/`tone`/`label`. Empty + error states per filter. Unread badge reactive with 99+ cap and optimistic mark-as-read. **Completion wave** — filter tabs (All/Unread/Mentions/Comments/Reactions/Spaces) now swap content reactively via PageRouter's HX-Request partial-render path (no full page reload, cmd-click still works, popstate reloads cleanly). Document title is reactive: `/notifications/` with unread reads "Notifications (3) · BuddyNext" (caps at "99+"); zero-unread reads the bare "Notifications · BuddyNext"; specialisation lives in PageRouter's existing `document_title_parts` block. Background unread-count poller (30s idle / 5s hot for 60s after a user action / paused while `document.hidden`) keeps the mobile-nav badge and any header pill accurate without revisiting the page. Pro realtime seam: `window.dispatchEvent(new CustomEvent('bn:notification:new'))` kicks the poller into hot mode and pulls fresh counts — Free has no producer so the event is a no-op until Pro P3.1 Soketi push is active. |
| 11 | Notification prefs (`/settings/notifications/`) | prod | New route registered alongside the existing notifications hub (PageRouter `bn_notif_section=prefs` query var, accessible at `/settings/notifications/` AND `/notifications/preferences/`). Page surfaces four sections: Channels (in-app / email / push / **sound** master toggles - push row hidden when `BuddyNextPro\Push\PushDispatcher` is not loaded; sound row plays a soft chime on `bn:notification:new` when `bnShellData.notifSoundUrl` is set), Activity types (accordion grouped by `NotificationPrefCatalogue::grouped()` covering Social graph / Feed activity / Spaces / Messages / Moderation / Growth and digests), Spaces you are in (per-space `all` / `mentions_only` / `none` chip-select saved instantly via POST `/me/space-notification-prefs`), Quiet hours (coming-soon placeholder). Sticky save bar matches the Profile edit pattern (dirty / saving / saved states + beforeunload guard). Optimistic UI with full rollback on REST 4xx/5xx + toast on success/failure. Document title now reads "Notification preferences · BuddyNext" (Wave A8/B1). `GET /me/notification-prefs` merges catalogue defaults so the response always carries one row per type; `PUT /me/notification-prefs` returns 422 on invalid `email_freq`. New REST: `GET + PUT /me/notification-channels` (usermeta `bn_channel_prefs`, now including the `sound` boolean), `GET + POST /me/space-notification-prefs`. Catalogue is filterable via `buddynext_notification_prefs_catalogue` so Pro / bridges register additional types. Settings link added to `/notifications/` page header; sidebar Preferences link now routes here. |
