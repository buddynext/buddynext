...
|---|---|---|---|
| BN announces itself | `mvs_buddynext_active → __return_true` | engine uses BN profile/identity | `WPMediaVerseBridge::init()` bails if `! class_exists('WPMediaVerse\Core\Plugin')` (`:48`) |
...
- **`mvs_comment_created` lightbox-comment sync (arg order FIXED 2026-06-09).** The bridge now hooks `mvs_comment_created` at priority 10/3 args as `sync_lightbox_comment( int $media_id, int $user_id, int $comment_id )` (`includes/Bridges/WPMediaVerseBridge.php:113,771`), matching the partner's first three positional args `do_action( 'mvs_comment_created', $media_id, $user_id, $comment_id, $content, $source )` (`wpmediaverse/includes/Social/CommentService.php`). The handler resolves the `wp_bn_posts` row via `JSON_CONTAINS(media_ids, media_id)`, dedups on (post, author, body), inserts a `wp_bn_comments` row, increments `comment_count`, and fires the canonical 4-arg `buddynext_comment_created`. Lightbox-comment → activity-feed sync is reliable; the earlier arg-order mismatch is resolved.
