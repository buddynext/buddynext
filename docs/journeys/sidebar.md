...
- **Cache-bust busts only the default-limit keys (by design)**: `WidgetService::suggested_follows()` writes under `suggested-v2:{user}:{limit}` and `joined_spaces()` under `spaces:{user}:{limit}`. `WidgetCache::invalidate_user()` deletes `suggested-v2:{user}:3` and `spaces:{user}:4` — the v2 key mismatch was fixed (the bust now targets the same `suggested-v2:` key the service writes), so the follow / unfollow / block hooks DO clear the suggested-follows cache at the rendered limit. Only non-default (admin-only) limits are left to self-heal at the 300 s `TTL_USER`, since the partial only ever renders limit 3 / 4. File: `includes/Sidebar/WidgetCache.php` lines 84-92.
