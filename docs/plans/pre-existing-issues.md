...
| PE-4 | both | Pre-commit hook inactive (`git config core.hooksPath` unset) → `bin/check.sh` does **not** run on commit. Commits rely on manual gate runs. | low (process) | **FIXED (2026-06-25)** — free `core.hooksPath` set to `.githooks` (pro was already set). Both repos now run `check.sh --staged --skip-audit` on commit. |
...
| PE-6 | free | Legacy `tests/Feed/PollServiceTest.php` carries 18 pre-existing WPCS errors (test methods lack docblocks). | low (test style) | **FIXED (2026-06-25)** — file-level `phpcs:disable` for the terse-test comment sniffs (repo precedent, e.g. DripEnrollmentServiceTest) + the one real array-spacing fix; 7/7 still pass. NB: this is a repo-wide pattern (~100 test files share it) and `tests/` is NOT in the `check.sh` full-mode scope, so the rest is cosmetic, not a gate failure. |
