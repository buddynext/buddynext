...
| U6 | **Hero title wrap fix** — a two-word space name (e.g. "Design Critique") was crushing into a stacked column because `.bn-sh-hero__info` had `min-width:0` (collapsed under the action buttons on the same row). Set `min-width: min(100%, 16rem)` so the title takes priority and the actions wrap below when tight. | ✅ done + browser-verified | Title renders on one line (info 441px); actions wrap below on a narrow head; mobile unaffected. |
...
| F1 | **DRY follow-up:** the flat directory grid (`/spaces/` All view) still uses the inline card; only the sectioned My Spaces view uses the new `parts/space-directory-card.php`. Convert the flat grid to the part too. | ⏳ follow-up | Both views render from one card source. |
