...
| **Gap-audit** enforcement fixes | ✅ DONE — audit-confirmed | `can_invite()` honors `who_can_invite` (`SpaceMemberService.php:284`); `default_notification_pref` applied on join/request/invite (`:56/151/247/343`). 2026-06-30 connectivity audit confirmed both read+applied, no key mismatch. |
...
- ✅ Directory composite indexes (S3) — DONE (schema v12+v13, 2026-06-30). All three directory sorts are now index-backed, filesort-free at the 20-30k-spaces-per-site target (EXPLAIN: backward index scan): Popular → `dir_popular (parent_id, member_count)`, A→Z → `dir_name (parent_id, name)`, Newest → `dir_recent (parent_id, created_at)` (write-once index, zero maintenance). Justified by the owner's stated scale (members create spaces → 20-30k directory rows), not deferred. Also added: opt-in "Include sub-spaces" filter (roots-only default stays uncrowded + bounded) and a reactive pager that rebuilds for the filtered set (SSR page-links + indexed OFFSET; keyset reserved for deep-scroll feeds). **SKIPPED permanently:** the "Recently active" sort — a `last_activity_at` column would need maintenance on every space post (an ongoing background cost we chose to avoid); "Most active" removed from the dropdown (was popularity relabeled). Deferred: reactive pagination for the *search* path (search stays page-1). See [[buddynext-scale-target]].
