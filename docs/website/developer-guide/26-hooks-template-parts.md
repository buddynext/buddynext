...
- **For whole-template transformations, see the output filter.** The `buddynext_template_output` filter operates at the template-loader level, giving you the complete rendered HTML of any template (not just individual parts). This is the right seam for structural changes like adding `loading=lazy` to all images or injecting analytics snippets, where modifying every individual part would be repetitive. See the *Transform rendered output with a filter* section of the Child Theme Overrides page.
