...
4. A single report does not hide or delete anything. The content stays visible while it waits for a moderator. If enough different established members report the same post or comment, it is automatically put "Under review" and hidden from other members until a moderator decides (see the Auto-hide note below).
