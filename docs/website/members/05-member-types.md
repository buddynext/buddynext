...
Member types are the categories you define for the people in your community - Student, Mentor, Staff, Alumni, and so on. Each type can carry a colored badge, appear as a filter in the member directory, and optionally be picked by members themselves.
...
- **Badges only show when assigned.** A type you have created but not assigned to anyone shows no badges and an empty directory filter until at least one member has it. On a new site, create your types and assign a few before expecting the directory filter and badges to look populated.
