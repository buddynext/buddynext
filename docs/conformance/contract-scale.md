...
- **B1 — RESOLVED.** `on_post_created_in_space()` no longer enumerates in-request. It enqueues `buddynext_async_space_post_fanout` (Action Scheduler), which pages `bn_space_members` in **keyset** batches of `SPACE_FANOUT_BATCH` (200) via the PK `(space_id, user_id)` — `WHERE user_id > %d ... LIMIT %d`, no OFFSET — and re-enqueues from the last `user_id`. The worker BATCHES every per-recipient read (member + space-pref in the keyset query, blocks via `blocked_member_ids`, per-type `on_site` via `get_on_site_map`, and the group dedup) and bulk-INSERTs / bulk-UPDATEs the in-app rows, then fires `buddynext_notification_created` per row; email is split into a separate self-paginating `buddynext_async_space_post_emails` AS stage (`defer_email`). Free-only fan-out is constant per batch (~11 queries / 200 members) instead of ~6 per member. The AS-absent fallback drains every batch inline (previously it processed only the first batch). `NotificationListener.php` (`fan_out_space_post_batch`, `blocked_member_ids`, `async_space_post_emails`).
