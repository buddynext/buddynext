...
# 2. Drop dev / QA / docs — none of it is needed to RUN the plugin.
( cd "$STAGE" \
    && rm -rf tests docs audit bin plan .github node_modules \
    && rm -f ./*.sh .cert.json .contract-audit-baseline.json phpcs.xml* phpstan* \
       phpunit* .editorconfig .gitignore .gitattributes package.json package-lock.json )
