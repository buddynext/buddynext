...
**Integration shape (native, not embedded):** every integration is **API-level only**, and **all integration happens at the BuddyNext level** (BN is the integration layer). The partner plugin is a headless data/API provider; BuddyNext renders the data with **its own native components on its own BN routes** — `/messages/`, `/forums/`, `/courses/`, `/jobs/` are BuddyNext surfaces, not the partner's UI. **No 2nd-party screens, no link-outs to a partner's UI, no foreign CSS/JS on BN routes.**

The split between **our own products** and **3rd parties** is *ownership of polish*, not location. Third-party developers integrate at the BN level via the extension seams and own their own polish. **Our own suite products must be polished as part of the system from day 1**: the addon ships **data/logic only — zero UI/CSS/JS** on BN routes, and **BuddyNext owns the entire presentation centrally — one place to style, never CSS tweaked across 10 addon plugins. BN overs all itself.** Each app gets "their own page" in the sense of *its own BN section/URL rendered natively by BN* — never the partner plugin's separate screen. Proven for media and messages/group-messages (`includes/Media/` + native `dm-*`); every new first-party surface copies it and is built in Pro. The HOW is locked in `plan/native-integration-program.md`. Best-of-breed depth, one uniform BN feel on every BN URL.

**Member mental model — "one community, many sections":** members never see "13 plugins." They see one community with a single menu — Feed · Messages · Forums · Courses · Jobs · Events · Directory — each powered by a best-of-breed app's API but **rendered natively as a BuddyNext section on a BN URL** (never the partner's own screen). Enforced by:
