...
- **Global shell (#10) — WALKED, no fix.** Left rail, mobile bottom-nav (390px), theme toggle (dark verified via real toggle), right-sidebar widgets all exercised across every screen this session. Search overlay opens on `/` (centred command-palette, focused input, Esc to close, routes to `/activity/search/`). 0 console errors.
- **PARENT-THEME ROOT FIX (the "jumpy/box/invisible-text on hover" cascade).** Root cause: the active theme styled bare `<button>` at element specificity and swapped border+bg+text on hover/focus, boxing/shifting/hiding BN controls. Fixed at the theme level by wrapping the base button rules in `:where()` (zero specificity → app button systems override): **reign-theme 8.0.0** `b200a164e`, **buddyx 5.1.1** `b66121d`. BN-side defenses removed (`4c7f8a9`, `9d2eca6`). Also: About tab (`b00179c`), Discard→trash icon (`fb7544f`), soft same-tone tab hover + underline focus, Explore 404 (`574a769`).
...
- **Explore / Search / Hashtag / Bookmarks / Leaderboard (#9) — COMPLETE.** Real bug fixed: Explore filter chips (People/Posts/Spaces/Media) + the Explore search bar routed to `/search/` — a 404 — instead of `/activity/search/` (`574a769`, `BN_SEARCH_PATH`). Search results page, Hashtag (header/follow/sort/related/contributors), Bookmarks, Leaderboard (rank/points/level + progress + period-link tabs + ranked list) all render + function cleanly, 0 console errors.
- **Profile edit / settings (#8) — WALKED, no critical fix.** Huge but well-organized: edit hero + live preview + field-visibility, Basic Info, Social, Work/Education repeaters, Skills, Privacy (6 audience selects + 4 toggles), Appearance (Theme/Text-size segmented — active state correctly shown, plan ⚠ resolved), Notification prefs, Account (slug + inline email/password Change forms expand, 2FA, sessions), Danger zone. Sticky `position:fixed` save bar. Desktop + 390px: no overflow, no fields overflowing, save bar in view. Deferred-minor: 2FA backup codes have no copy/download button; avatar-crop has no upload progress.
- **Spaces (#7) — WALKED, no critical fix.** Directory (search + type chips + sort + Join + sidebar), create-space modal (name/slug/type/description, maxlengths enforced), single space (hero + stats + Feed/Members/Media/About tabs + join-gate + sidebar), space settings (full tab set General…Brand + uploads). All mature, 0 console errors. Deferred-minor (not blocking): create-space description has no live char counter; card member-count not reactive on join; settings inline + sticky save buttons coexist. Residual not exhaustively walked: space moderation tab, space mobile/dark, member-management destructive confirms.
